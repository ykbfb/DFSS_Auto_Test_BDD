...
def step_createBillReceipt(context,trans_type):
    ...
    if trans_type == '意向金':
        bill.createBill(data.getClient('新客户')['ContractCode'])
    elif trans_type == '服务费':
        bill.createServiceBill(data.getClient('新客户')['ContractCode'])
    else:
...
def step_verifyBillReceiptCreateSucess(context):
    bill.backtoBillList()
    bill.searchBillReceipt(data.getClient('新客户')['ContractCode'])
...
def step_caculateBillReceipt(context):
    bill.caculateBill(data.getClient('新客户')['ContractCode'])
    bill.setWaitTime(2)
    functions.insert_img(context.driver, "caculateBill_"+current_time+".png")

@Then('收款单结算成功')
def step_BillCaculateSucess(context):
    bill.searchBillReceipt(data.getClient('新客户')['ContractCode'], '已结算')
